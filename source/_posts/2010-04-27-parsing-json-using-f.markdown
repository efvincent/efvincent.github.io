...
author: EFVincent
comments: true
date: 2010-04-27 18:29:49+00:00
layout: post
slug: parsing-json-using-f
...
Things are a bit more interesting at lines 28, 31, and 34. Line 28 detects the beginning of a string, and starts a new recursive thread with the _parseString_ function, the signature of which is (string –> char list –> string * char list). So the accumulator is a string, the “work” is being done on a char list, to which is passed our source char list, and the return is a tuple of string * char list, which is our parsed string and the rest of the source char list. One (of I’m sure **many**) optimizations that could be made is to use a mutable StringBuilder as the accumulator in parseString.
